...
abstract class LayoutPlatformFormat {
  const LayoutPlatformFormat();
  ...
  final Map<LayoutPlatformBreakpoint, double> breakpoints = const {};
  ...
  @protected
  LayoutPlatformBreakpoint breakpointForWidth(double width) {
    // Iterate all breakpoints from biggest to smallest
    for (final breakpoint in LayoutPlatformBreakpoint.values.reversed) {
      if (breakpoints[breakpoint] != null &&
          width >= breakpoints[breakpoint]!) {
        return breakpoint;
      }
    }
    return LayoutPlatformBreakpoint.watch;
  }
}
